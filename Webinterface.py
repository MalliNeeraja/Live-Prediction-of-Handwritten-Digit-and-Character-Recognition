#final code for digits and characters
...
digit_weights_file = '/content/improved1.weights.h5'
if os.path.exists(digit_weights_file):
    ...
    print(f"{digit_weights_file} not found. Please upload it.")
...
character_weights_file = '/content/checkpoint.weights.h5'
if os.path.exists(character_weights_file):
    ...
    print(f"{character_weights_file} not found. Please upload it.")
...
def correct(data_url, true_label, mode):
    ...
    filename = "custom_digits.h5" if is_digit else "custom_characters.h5"
    save_to_custom_dataset(img[0], true_label, filename, is_digit)
    return predict(data_url, mode)

# --- Load existing custom datasets ---
load_custom_dataset("custom_digits.h5", is_digit=True)
load_custom_dataset("custom_characters.h5", is_digit=False)
...
class CustomHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            ...
            try:
                with open('/content/backgroundimage.jpg', 'rb') as f:
                    self.send_response(200)
...
ngrok.kill()
time.sleep(1)
ngrok.set_auth_token('2vOZqDZR506sTtASAU3CAHuoDaY_7StwmEvcGmsjS9f5MCLQi')

try:
    tunnels = ngrok.get_tunnels()
    public_url = tunnels[0].public_url if tunnels else ngrok.connect(port).public_url
except Exception as e:
